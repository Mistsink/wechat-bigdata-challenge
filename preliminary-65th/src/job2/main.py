...
def train_and_validate(args, train_dataloader, val_dataloader, fold=0):
    ...
    for epoch in range(args.max_epochs):
        ...
        validate_and_save_model(args, val_dataloader, model, best_score, epoch, validate, logger, model_save_path)
    
    if swa != None:
        loss, results = validate(swa.model(), val_dataloader)
        results = {k: round(v, 4) for k, v in results.items()}
        logger.info(f"SWA : Loss {loss:.3f}, F1 {results['mean_f1']}")
        
        swa_save_path = f'{args.path_out}job2_swa_{fold}.bin'
